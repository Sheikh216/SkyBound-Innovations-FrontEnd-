...
const Cart = () => {
  ...
  const handlePayment = async () => {
    ...
    const response = await axios.post('/payment',{
      ...
      purpose: "package_booking",
      package_name: cartItems[0].packageName,
      airline_name: cartItems[0].airlines,
      price: cartItems[0].price,
      seats: cartItems[0].seats[0],
      flight_id: cartItems[0].flight_id
    }, {
     headers: {
  ...
  return (
    <div className="flex flex-col max-w-3xl p-6 space-y-4 sm:p-10 dark:bg-gray-50 dark:text-gray-800">
              ...
              <div className="flex flex-col justify-between w-full pb-4">
                <div className="flex justify-between w-full pb-2 space-x-2">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm line-through dark:text-gray-400">{item.discountedPrice}৳</p>
                  </div>
                </div>
                <div className="flex text-sm divide-x">
                  <button type="button" className="flex items-center px-2 py-1 pl-0 space-x-1" onClick={() => removeFromCart(item.packageName)}>
      ...
      <div className="space-y-1 text-right">
        <p>Total amount: <span className="font-semibold">{calculateTotal()}  ৳</span></p>
      ...
      <div className="flex justify-end space-x-4">
        ...
        <button type="button" onClick={()=> handlePayment()} className="px-6 py-2 border rounded-md dark:bg-violet-600 dark:text-gray-50 dark:border-violet-600"><span className="sr-only sm:not-sr-only">Continue to</span> Pay</button>
      </div>
    </div>
  );
};
